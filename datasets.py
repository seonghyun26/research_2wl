...
class BaseGraph:
    ...
    def preprocess(self):
        ...
        self.edge_attrs = [
            torch.ones_like(self.edge_indexs[i][0], dtype=torch.float) for i in range(3)
        ]
        ...
        pos_edges = [
            self.edge_pos[:, : self.num_pos[0]],
            self.edge_pos[:, self.num_pos[0] : self.num_pos[0] + self.num_pos[1]],
            self.edge_pos[:, -self.num_pos[2] :],
        ]
        neg_edges = [
            self.edge_neg[:, : self.num_neg[0]],
            self.edge_neg[:, self.num_neg[0] : self.num_neg[0] + self.num_neg[1]],
            self.edge_neg[:, -self.num_neg[2] :],
        ]

        pred_edges = [neg_edges[0]] + [
            torch.cat((pos_edges[i], neg_edges[i]), dim=1) for i in range(1, 3)
        ]
        ...
        self.ys = [
            torch.zeros((neg_edges[0].shape[1], 1), device=self.edge_pos.device)
        ] + [
            torch.cat(
                (
                    torch.ones(
                        (pos_edges[i].shape[1], 1),
                        dtype=torch.float,
                        device=self.edge_pos.device,
                    ),
                    torch.zeros(
                        (neg_edges[i].shape[1], 1),
                        dtype=torch.float,
                        device=self.edge_pos.device,
                    ),
                )
    ...
    def split(self, split: int):
        return (
            self.x[split],
            self.node_attr,
            self.edge_indexs[split],
            self.edge_attrs[split],
            self.pos1s[split],
            self.ys[split],
            self.ei2s[split],
        )
    ...
    def setPosDegreeFeature(self):
        ...
        self.x = [degree(self.edge_indexs[0], self.num_nodes) for i in range(0, 2)] + [
...
def load_dataset(name, pattern, trn_ratio=0.8, val_ratio=0.05, test_ratio=0.1):
    if name in [
        "arxiv",
        "Celegans",
        "Ecoli",
        ...
        return BaseGraph(x, node_attr, edge_pos, edge_neg, num_pos, num_neg, pattern)
    else:
