...
class WLNet(torch.nn.Module):
    def __init__(
        ...
        self.embedding = torch.nn.Sequential(
            torch.nn.Embedding(max_x + 1, hidden_dim_1), torch.nn.Dropout(p=dp1)
        )
        # self.embedding = nn.Embedding(max_x + 1, latent_size_1)

        self.nconvs = nn.ModuleList(
            [Convs(hidden_dim_1, hidden_dim_1, dp2, ln1, act1)]
            + [
                Convs(hidden_dim_1, hidden_dim_1, dp2, ln2, act2)
        ...
        self.h_1 = Seq(
            [relu_lin(input_edge_size + 1, hidden_dim_2, dp3, ln3, act3)]
            + [
                relu_lin(hidden_dim_2, hidden_dim_2, dp3, ln3, act3)
        ...
        self.g_1 = Seq(
            ...
            + [
                relu_lin(hidden_dim_2, hidden_dim_2, dp3, ln4, act4)
    ...
    def __init__(
        ...
        if use_node_feat:
            ...
            self.emb = nn.Sequential(
                nn.Embedding(max_x + 1, channels_1wl),
        ...
        self.conv1s = nn.ModuleList(
            [
                relu_conv(channels_1wl, channels_1wl, dp_1wl0, act0)
...
class FWLNet(nn.Module):
    def __init__(
        ...
        self.layer2 = layer2
        self.subgraph = subgraph
        self.alphaWeight = nn.Parameter(torch.ones(2))
        ...
        self.embedding = nn.Sequential(
            nn.Embedding(max_x + 1, hidden_dim_1), nn.Dropout(p=dp1)
        )
        ...
        relu_sage_end = lambda a, b, dp: Seq(
            [GCNConv(a, b), GraphNorm(b), nn.Dropout(dp, inplace=True)]
        )
        if not act:
            self.nconvs = nn.ModuleList(
    ...
    def forward(self, x, ei, pos, ei2=None, test=False):
        ...
        # NOTE: Making A, in range(n) hop neighbor
        x = x.reshape(n, n, -1)
        for i in range(3):
        ...
        # NOTE: MakeB & update A, layer2 default 1
        for i in range(self.layer2):
            ...
            elif self.subgraph == "cycle":
                # print("Cycle")
                x1_cyc = self.mlps_1[i](x).permute(2, 1, 0)
                x2_cyc = self.mlps_2[i](x).permute(2, 1, 0)
            elif self.subgraph == "outgoing":
                # print("Incoming")
                x1 = self.mlps_1[i](x).permute(2, 0, 1)
                x2 = self.mlps_2[i](x).permute(2, 1, 0)
            elif self.subgraph == "incoming":
                # print("Outgoing")
                x1 = self.mlps_1[i](x).permute(2, 1, 0)
                x2 = self.mlps_2[i](x).permute(2, 0, 1)
            elif self.subgraph == "alpha" or self.subgraph == "mean":
                edge_1 = self.mlps_1[i](x)
                x1_out = edge_1.permute(2, 0, 1)
                x1_in = edge_1.permute(2, 1, 0)
                edge_2 = self.mlps_2[i](x)
                x2_out = edge_2.permute(2, 1, 0)
                x2_in = edge_2.permute(2, 0, 1)
            else:
                # just in case
                x1 = self.mlps_1[i](x).permute(2, 0, 1)
                x2 = self.mlps_2[i](x).permute(2, 0, 1)

            # NOTE: Sigma done here, with 2 MLPs
            if self.subgraph == "alpha":
                x_in = torch.cat([x, (x1_in @ x2_in).permute(1, 2, 0)], -1)
                x_out = torch.cat([x, (x1_out @ x2_out).permute(1, 2, 0)], -1)
                x = (self.alphaWeight[0] * x_in + self.alphaWeight[1] * x_out) / 2
            elif self.subgraph == "cycle":
                x = torch.cat([x, (x1_cyc @ x2_cyc).permute(1, 2, 0)], -1)
            elif self.subgraph == "mean":
                a = x.clone().detach()
                b = x.clone().detach()
                c = x.clone().detach()
                x_in = torch.cat([a, (x1_in @ x2_in).permute(1, 2, 0)], -1)
                x_out = torch.cat([b, (x1_out @ x2_out).permute(1, 2, 0)], -1)
                x_cyc = torch.cat([c, (x1_out @ x2_in).permute(1, 2, 0)], -1)
                x = torch.cat([x, (x1_in @ x2_out).permute(1, 2, 0)], -1)
                x = (x + x_in + x_out + x_cyc) / 4
            else:
                x = torch.cat([x, (x1 @ x2).permute(1, 2, 0)], -1)
            x = self.mlps_3[i](x)
        x = (
            (x * x.permute(1, 0, 2)).reshape(n * n, -1)
            if self.mul_pool
            else (x + x.permute(1, 0, 2)).reshape(n * n, -1)
        )
        x = x[pos[:, 0] * n + pos[:, 1]]
        x = self.lin_dir(x)
        return x
...
class LocalFWLNet(nn.Module):
    def __init__(
        ...
        if use_feat:
            ...
            self.feat = nn.parameter.Parameter(feat, requires_grad=False)
        self.embedding = nn.Sequential(
            nn.Embedding(max_x + 1, hidden_dim_1wl), nn.Dropout(p=dp_emb)
        )
        if not use_appnp:
            self.nconvs = nn.ModuleList(
                [
                    relu_sage(
                        input_node_size, hidden_dim_1wl, dp_1wl, ln_1wl, gn_1wl, act_1wl
                    )
                ]
                + [
                    relu_sage(
                        hidden_dim_1wl, hidden_dim_1wl, dp_1wl, ln_1wl, gn_1wl, act_1wl
                    )
                    for _ in range(layer1 - 1)
                ]
            )
        ...
        if reduce_feat:
            ...
            self.lin1 = nn.Sequential(
                nn.Dropout(dp_lin0),
                relu_lin(
                    input_node_size, hidden_dim_1wl, dp_lin1, ln_lin, False, act_lin
                ),
            )
        else:
        ...
        self.mlps_1 = relu_lin(
            input_edge_size * 2, hidden_dim_2wl, dp_2wl0, ln_2wl0, False, act_2wl0
        )
        self.mlps_2 = nn.ModuleList(
            [
                relu_lin(
    ...
    def forward(self, x, ei, pos, ei2=None, test=False):
        t0 = time.time()
        edge_index = ei
        edge_index_reverse_1 = edge_index.clone().detach()
        edge_index_reverse_2 = edge_index.clone().detach()
        edge_index_reverse_1[0] = edge_index[1]
        edge_index_reverse_1[1] = edge_index[0]
        edge_index_reverse_2[0] = edge_index[1]
        edge_index_reverse_2[1] = edge_index[0]
        ...
        val = torch.cat(
            [x[edge_index[0]], x[edge_index[1]]], 1
        )  # colx = x.unsqueeze(0).expand(n, -1, -1).reshape(n * n, -1)
        ...
        for i in range(self.layer3):
            ...
            if self.subgraph == "path":
                ...
                x = sparse_bmm(
                    edge_index_reverse_1,
                    x,
                    edge_index_reverse_2,
                    mul,
                    n,
                    fast=self.fast,
                )
                ...
                x = sparse_bmm(
                    edge_index_reverse_1, x, edge_index, mul, n, fast=self.fast
                )
            elif self.subgraph == "outgoing":
                x = sparse_bmm(
                    current_edges, x, edge_index_reverse_2, mul, n, fast=self.fast
                )
            else:
                x = sparse_bmm(current_edges, x, edge_index, mul, n, fast=self.fast)

            current_edges, value = sparse_cat(
                x, edge_index, torch.ones((edge_index.shape[1], 1), device=x.device)
            )
        ...
        sm = (
            torch.sparse.FloatTensor(
                torch.cat(
                    [current_edges[1].unsqueeze(0), current_edges[0].unsqueeze(0)], 0
                ),
                x,
                torch.Size([n, n, x.shape[-1]]),
            )
            .coalesce()
            .values()
        )
        ...
        x = (
            self.lin_dir(x)
            if not self.sum_pooling
            else torch.sum(x, dim=-1, keepdim=True)
        )
        # t6 = time.time()
        # print(t1 - t0)
        # print(t2 - t1)
...
class Net_cora(nn.Module):
    def __init__(
        ...
        relu_lin = lambda a, b, dp: nn.Sequential(
            nn.Linear(a, b),
            nn.LayerNorm(b, elementwise_affine=use_affine) if ln2 else nn.Identity(),
            nn.Dropout(p=dp, inplace=True),
        ...
        relu_norm_lin = lambda a, b, dp: nn.Sequential(
            nn.Linear(a, b),
            nn.LayerNorm(b, elementwise_affine=use_affine) if ln3 else nn.Identity(),
            nn.Dropout(p=dp, inplace=True),
